			...
			if($WinTeam!=='yellow')
			  {					
				if($GameData[$j]['team']==$WinTeam)
				  {
					$numPlayersInWinTeam++; 
					$SumReiting2Win = $SumReiting2Win + ($Game[$j+1]['CtfReiting'] + 100)*($Game[$j+1]['CtfReiting'] + 100);
				  }
				else
				  { 
					$SumReiting2Lose = $SumReiting2Lose + ($Game[$j+1]['CtfReiting'] + 100)*($Game[$j+1]['CtfReiting'] + 100);
				  }
			  }
			else
			  {
	  ...
	  for($j=1;$j<=$GameNumRows;$j++)
		{
		  ...
		  if($Game[$j]['team']==$WinTeam)
			{
			  $ChangeReiting = $AddAll*$Table[$GameNumRows][$j]/100-$AddAllWin*($Game[$j]['CtfReiting']+100)*($Game[$j]['CtfReiting']+100)/$SumReiting2Win*$Game[$j]['timeCoef'];
			  $Game[$j]['CtfWin']++;
			  $ChangeReiting = $ChangeReiting + $AddWin/$numPlayersInWinTeam;
			}
		  else
			{
			  $ChangeReiting = $AddAll*$Table[$GameNumRows][$j]/100-$AddAllLose*($Game[$j]['CtfReiting']+100)*($Game[$j]['CtfReiting']+100)/$SumReiting2Lose*$Game[$j]['timeCoef'];
			}					
		  If($ChangeReiting<0)
			{
			  $ChangeReiting=$ChangeReiting*(1-exp((100-$Game[$j]['CtfReiting'])/100));
			}
		  else
			{
			  $ChangeReiting=$ChangeReiting*(1-exp(($Game[$j]['CtfReiting']-1500)/180));
			}
		  $ChangeReiting = floor($ChangeReiting);
		  if (($ChangeReiting < 0) and ($Game[$j]['team']==$WinTeam)) $ChangeReiting = 0;
			...
			if($WinTeam!=='yellow')
			  {					
				if($GameData[$j]['team']==$WinTeam)
				  {
					$numPlayersInWinTeam++; 
					$SumReiting2Win = $SumReiting2Win + ($Game[$j+1]['TdmReiting'] + 100)*($Game[$j+1]['TdmReiting'] + 100);
				  }
				else
				  { 
					$SumReiting2Lose = $SumReiting2Lose + ($Game[$j+1]['TdmReiting'] + 100)*($Game[$j+1]['TdmReiting'] + 100);
				  }
			  }
			else
			  {
	  ...
	  for($j=1;$j<=$GameNumRows;$j++)
		{
		  ...
		  if($Game[$j]['team']==$WinTeam)
			{
			  $ChangeReiting = $AddAll*$Table[$GameNumRows][$j]/100-$AddAllWin*($Game[$j]['TdmReiting']+100)*($Game[$j]['TdmReiting']+100)/$SumReiting2Win*$Game[$j]['timeCoef'];
			  $Game[$j]['TdmWin']++;
			  $ChangeReiting = $ChangeReiting + $AddWin/$numPlayersInWinTeam;
			}
		  else
			{
			  $ChangeReiting = $AddAll*$Table[$GameNumRows][$j]/100-$AddAllLose*($Game[$j]['TdmReiting']+100)*($Game[$j]['TdmReiting']+100)/$SumReiting2Lose*$Game[$j]['timeCoef'];
			}					
		  If($ChangeReiting<0)
			{
			  $ChangeReiting=$ChangeReiting*(1-exp((100-$Game[$j]['TdmReiting'])/100));
			}
		  else
			{
			  $ChangeReiting=$ChangeReiting*(1-exp(($Game[$j]['TdmReiting']-1500)/180));
			}
		  $ChangeReiting = floor($ChangeReiting);	
		 if (($ChangeReiting < 0) and ($Game[$j]['team']==$WinTeam)) $ChangeReiting = 0;
